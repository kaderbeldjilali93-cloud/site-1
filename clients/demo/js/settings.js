...
window.renderSettingsAccount = async function() {
    const dynamicContent = document.getElementById('dynamic-content');
    if(!dynamicContent) return;
    dynamicContent.innerHTML = '<div class="flex h-full items-center justify-center"><div class="animate-spin rounded-full h-12 w-12 border-t-4 border-b-4 border-brand"></div></div>';
    
    let adminPass = '123';
    let kitchenPass = 'k123';
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.results && data.results.length > 0) {
                const row = data.results[0];
                adminPass = row.AdminPass ?? '123';
                kitchenPass = row.KitchenPass ?? 'k123';
            }
        }
    } catch (error) {
        console.warn("Failed to fetch settings:", error);
    ...
    container.innerHTML = `
        <div class="bg-gray-800 border border-gray-700 rounded-2xl shadow-xl p-6 md:p-8">
            <div class="space-y-6">
                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-2">كلمة سر المدير (Admin)</label>
                    <input type="text" id="setting-admin-pass" value="${adminPass}" class="w-full bg-gray-900 border border-gray-600 text-white rounded-lg px-4 py-3 focus:ring-2 focus:ring-brand outline-none transition" placeholder="مثال: 1234">
                </div>
                
                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-2">كلمة سر المطبخ (KDS)</label>
                    <input type="text" id="setting-kitchen-pass" value="${kitchenPass}" class="w-full bg-gray-900 border border-gray-600 text-white rounded-lg px-4 py-3 focus:ring-2 focus:ring-brand outline-none transition" placeholder="مثال: k123">
                </div>

                <div class="pt-4 mt-6 border-t border-gray-700">
                    <button id="btn-save-account" onclick="window.saveAccountSettings()" class="w-full bg-brand hover:bg-brand-dark text-black font-bold text-lg py-4 rounded-xl transition shadow-lg transform hover:scale-[1.01]">
                        حفظ إعدادات الحساب
                    </button>
                </div>
            </div>
...
window.saveAccountSettings = async function() {
    ...
    const adminPassVal = document.getElementById('setting-admin-pass').value.trim();
    const kitchenPassVal = document.getElementById('setting-kitchen-pass').value.trim();
    ...
    try {
        ...
        if (!settingsResponse.ok) throw new Error("فشل جلب الإعدادات");
        
        const settingsData = await settingsResponse.json();
        const payload = {
            "AdminPass": adminPassVal,
            "KitchenPass": kitchenPassVal
        };
        ...
        if (settingsData.results && settingsData.results.length > 0) {
            ...
            if (!updateResponse.ok) throw new Error("فشل تحديث الإعدادات");
        } else {
            const createResponse = await fetch(`https://baserow.vidsai.site/api/database/rows/table/${SETTINGS_TABLE_ID}/?user_field_names=true`, {
                method: 'POST',
                headers: { 
